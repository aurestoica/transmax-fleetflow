                ...
                <div className="flex items-start justify-between gap-4">
                  <div className="flex-1 min-w-0">
                    ...
                    <div className="flex items-center gap-4 text-xs text-muted-foreground mt-2">
                      {trip.clients?.company_name && <Link to={`/clients/${trip.client_id}`} className="text-primary hover:underline" onClick={e => e.stopPropagation()}>📦 {trip.clients.company_name}</Link>}
                      {trip.drivers?.full_name && <Link to={`/drivers/${trip.driver_id}`} className="text-primary hover:underline" onClick={e => e.stopPropagation()}>👤 {trip.drivers.full_name}</Link>}
                      {trip.vehicles?.plate_number && <Link to={`/vehicles/${trip.vehicle_id}`} className="text-primary hover:underline" onClick={e => e.stopPropagation()}>🚛 {trip.vehicles.plate_number}</Link>}
                    </div>
                  </div>
