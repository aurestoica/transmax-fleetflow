          ...
          {editingTrip ? (
            ...
            <>
              <div className="space-y-3 text-sm">
                ...
                {trip.clients?.company_name && <div className="flex items-center gap-3"><Package className="h-4 w-4 text-muted-foreground" /><span>Client: <Link to={`/clients/${trip.client_id}`} className="text-primary hover:underline">{trip.clients.company_name}</Link></span></div>}
                {trip.drivers?.full_name && <div className="flex items-center gap-3"><User className="h-4 w-4 text-muted-foreground" /><span>Șofer: <Link to={`/drivers/${trip.driver_id}`} className="text-primary hover:underline">{trip.drivers.full_name}</Link></span></div>}
                {trip.vehicles?.plate_number && <div className="flex items-center gap-3"><Truck className="h-4 w-4 text-muted-foreground" /><span><Link to={`/vehicles/${trip.vehicle_id}`} className="text-primary hover:underline">{trip.vehicles.plate_number} - {trip.vehicles.model}</Link></span></div>}
                {trip.trailers?.plate_number && <div className="flex items-center gap-3"><Truck className="h-4 w-4 text-muted-foreground" /><span>Remorcă: <Link to={`/trailers/${trip.trailer_id}`} className="text-primary hover:underline">{trip.trailers.plate_number}</Link></span></div>}
