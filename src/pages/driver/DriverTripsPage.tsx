...
export default function DriverTripsPage() {
  ...
  return (
    <div>
      ...
      {trips.length === 0 ? (
        <div className="text-center py-12 text-muted-foreground">Nu ai curse active</div>
      ) : (
        <div className="space-y-4">
          {trips.map(trip => {
            const nextAction = statusActions.find(a => a.from === trip.status);
            return (
              <div key={trip.id} className="bg-card rounded-xl border p-4 space-y-3" style={{ boxShadow: 'var(--shadow-card)' }}>
                ...
                {nextAction && (
                  <Button
                    ...
                    onClick={() => changeStatus(trip.id, nextAction.to, t(nextAction.key))}
                  >
                    {t(nextAction.key)}
                  </Button>
                )}
